...
RUN	apt-get update \
#	&& apt-get install gnupg openjdk-8-jre-headless curl gdal-bin tesseract-ocr tesseract-ocr-all ffmpeg -y \
	&& apt-get install gnupg openjdk-8-jre-headless curl gdal-bin ffmpeg -y \
...
EXPOSE 9998
ENTRYPOINT java -jar /tika-server-${TIKA_VERSION}.jar -h 0.0.0.0 --config=tika-config.xml -enableUnsecureFeatures -enableFileUrl